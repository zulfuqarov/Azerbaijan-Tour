...
mongoose.connect(
    process.env.MadinaTravleConnectLink,
    {
...
app.delete("/Delete-Travle/:id", async (req, res) => {
    ...
    try {
        ...
        if (!DeleteTravle) {
            ...
            res.status(200).json({ message: "Travle başarıyla silindi", deletedItem: DeleteTravle });
        }
    } catch (error) {
        console.log(error)
    }
...
app.post("/Register-Admin", async (req, res) => {
    try {
        if (
            ...
            const AdminCheckEmail = await AdminRegister.findOne({ email: req.body.InputEmail })
            if (AdminCheckEmail) {
                return (
                    res.status(400).json({ message: "This email is already available/existing." })
                )
            }
            ...
            return (
                res.status(400).json({ message: "Butun yerleri doldurun" })
            )
        }
    } catch (error) {
        console.log(eroor)
...
app.post("/Admin-Sign", async (req, res) => {
    try {
        const AdminSignEmail = await AdminRegister.findOne({ email: req.body.InputAdminEmailSign })
        if (!AdminSignEmail) {
            return (
                res.status(400).json({ message: "Email ve Kodu tekrar yoxlayin!!" })
            )

        }
        const AdminSignPassword = await bcrypt.compare(req.body.InputAdminPassword, AdminSignEmail.password)
...
const CheckToken = async (req, res, next) => {
    ...
    if (!token) {
        return (
            res.status(401).json({ message: "Kimlik dogrulamasi ugursuz oldu... Yeniden deneyin" })
        )
    }
    try {
        ...
        if (error instanceof jwt.TokenExpiredError) {
            return res.status(401).json({ message: "JWT süresi doldu. Lütfen tekrar oturum açın." });
        } else {
...
app.get("/Travle-Info/:id", async (req, res) => {
    ...
    try {
        ...
        if (!travleInfo) {
            return (
                res.status(400).json({ message: "Travle tapilmadi" })
            )
        }
        res.json(travleInfo)
    } catch (error) {
...
app.get("/More-Travle-Info/:id", async (req, res) => {
    ...
    try {
        ...
        if (!MoreInfoTravle) {
            return (
                res.status(400).json({ message: "More Travle Info tapilmadi" })
            )
        }
...
app.get("/Map/:id", async (req, res) => {
    ...
    try {
        ...
        const cvb = await axios.get(`https://nominatim.openstreetmap.org/search?format=json&q=${LocationId.Location}`)
...
app.get("/Map", async (req, res) => {
    ...
    try {
        if (locations) {
            const cvb = await axios.get(`https://nominatim.openstreetmap.org/search?format=json&q=${locations}`)
            res.status(200).json(cvb.data)
        }
        else {
...
app.get("/Blog-Comment", async(req,res)=>{
    ...
    try {
        ...
        if(!BlogCommentGet || BlogCommentGet.length  === 0){
            return(
                res.status(400).json({message: "Blog Comment tapilmadi!"})
            )
        }
